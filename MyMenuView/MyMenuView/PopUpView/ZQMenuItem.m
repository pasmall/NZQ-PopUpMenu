...
const CGFloat kZQMenuItemMargin = 5.f;


@interface ZQMenuItem ()
...
- (void)layoutSubviews{
    ...
    if (self.image && !self.title) {
        ...
        height               = ceil(MIN(self.image.size.height, self.height - 2*kZQMenuItemMargin));
        initX                = kZQMenuItemMargin;
    ...
    if (self.title && !self.image) {
        ...
        width            = ceil(textSize.width) + kZQMenuItemMargin;
    ...
    if (self.image && self.title) {
        ...
        initX                = width + 2 * kZQMenuItemMargin;
        initY                = 0;
...
- (CGSize)intrinsicContentSize
{
    ...
    if (self.title) {
        CGSize textSize = [self.title boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width*0.5, MAXFLOAT)
                                                ...
                                                attributes:@{NSFontAttributeName:kLabelFont}
                                                   context:nil].size;
        itemWidth  += ceil(textSize.width) + 2 * kZQMenuItemMargin;
        itemHeight += ceil(textSize.height) + 6;
    }
    
    if (self.image) {
        itemWidth += ceil(self.image.size.width) + 2 * kZQMenuItemMargin;
        itemHeight = MAX(ceil(self.image.size.height), itemHeight);
    }
    
    if (self.title && self.image) {
        itemWidth -= kZQMenuItemMargin;
    }
...
- (ZQMenu *)findMenuView
{
    ...
    while (![ancestorView isKindOfClass:[ZQMenu class]]) {
        ancestorView = ancestorView.superview;
    }
    
    return (ZQMenu *)ancestorView;
}
...
- (void)itemToggle
{
    ZQMenu *menuView = [self findMenuView];
